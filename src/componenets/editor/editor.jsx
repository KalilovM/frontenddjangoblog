...
import {
    ...
    EDITOR_TOOLS,
    EDITTOR_HOLDER_ID,
} from "../../constants/editorjs/editorjs";
import ImageTool from "@editorjs/image";
import styles from "./editor.module.scss";
import BottomBar from "./bottomBar/bottomBar";
import { Instance, InstanceApi } from "../../services/axios/axios";
import Layout from "./statuses/layout";
import auth from "../../store/auth";
import { useNavigate } from "react-router-dom";

// TODO here will be colored but I'm not sure
const Editor = () => {
                    ...
                    config: {
                        uploader: {
                            async uploadByFile(file) {
                                ...
                                setImages([
                                    ...images,
                                    response.data.file.url.match(/\w+\.[0-9a-z]+$/i)[0],
                                ]);
    ...
    return isSuccess ? (
        ...
        <div className={styles.container}>
            <section id={EDITTOR_HOLDER_ID} className={styles.content}></section>
            <BottomBar
                buttons={[
                    { onclick: Settings, name: "Перейки к следующим настройкам" },
                ]}
            />
        </div>
    ) : (
        <div className={styles.container}>
            <div className={styles.content}>
                ...
                <div className={styles.inputs}>
                    <label htmlFor="short_desc">Короткое превью текст</label>
                    <textarea
                        rows={1}
                        value={shortDesc}
                        onChange={AutoHeight}
